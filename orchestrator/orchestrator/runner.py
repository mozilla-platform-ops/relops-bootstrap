...
Config (env):
  ...
  REPROVISION_RUNNER_TOKEN  shared token; must match Hangar's settings.reprovision_runner_token
...
class Config:
    def __init__(self) -> None:
        ...
        if not self.api or not self.token:
            sys.exit("set HANGAR_API_URL and REPROVISION_RUNNER_TOKEN")

    @property
    def headers(self) -> dict[str, str]:
        return {"X-Reprovision-Runner-Token": self.token, "X-Reprovision-Runner-Id": self.runner_id}
...
def main() -> None:
    cfg = Config()
    print(f"reprovision-runner [{cfg.runner_id}] → {cfg.api} (poll {cfg.poll}s)")
