...
def test_mint_is_idempotent_when_already_enabled():
    ...
    with (
        ...
        patch(
            "orchestrator.workflow.ssh.secure_token_status",
            return_value="ENABLED for user admin",
        ),
...
def test_mint_logs_in_then_verifies_enabled():
    ...
    with (
        ...
        patch(
            "orchestrator.workflow.ssh.secure_token_status",
            side_effect=lambda *_: next(statuses),
        ),
        patch("orchestrator.workflow.ssh.password_login") as login,
    ):
        workflow.step_mint(_ctx())
...
def test_escrow_bst_is_non_interactive():
    ...
    assert (
        "s3cr3t-pw" in install
    )  # the resolved password is passed, not an empty string
...
def test_escrow_bst_ssh_error_does_not_leak_password():
    ...
    def boom(host, cmd, **_):
        ...
        raise ReprovisionError(
            f"remote command on {host} failed (exit 1): profiles: error"
        )

    with (
        patch("orchestrator.workflow.ssh.run", side_effect=boom),
        patch("orchestrator.workflow.ssh_admin_password", return_value="s3cr3t-pw"),
    ):
    ...
    assert (
        ei.value.__suppress_context__
    )  # `from None` — no chained exception leaks the cmd
...
def test_wipe_auto_escrows_bst_then_proceeds():
    ...
    def fake_run(fqdn, cmd, check=True, **kw):
        ...
        if "profiles status" in cmd:
            ...
            m.stdout = (
                b"escrowed to server: NO"
                if n["status"] == 1
                else b"escrowed to server: YES"
            )
        else:
...
def test_wipe_aborts_when_bst_unescrowable():
    ...
    with (
        ...
        run.return_value.returncode = (
            0  # ssh works; box genuinely has no BST and escrow won't take
        )
        run.return_value.stdout = b"profiles: Bootstrap Token escrowed to server: NO"
...
def test_wipe_forgets_stale_host_key_before_verifying():
    ...
    with (
        patch("orchestrator.workflow.ssh.forget_host_key") as forget,
        patch("orchestrator.workflow.ssh.run") as run,
        patch(
            "orchestrator.workflow.taskcluster.is_currently_busy", return_value=False
        ),
...
def _run_reprovision_capturing(**kwargs) -> list[str]:
    ...
    with (
        patch("orchestrator.workflow.resolve", return_value=_ctx()),
        ...
        patch("orchestrator.workflow.step_screencapture_grant", rec("screencapture")),
...
def test_reprovision_default_flow():
    ...
    assert not hasattr(
        workflow, "step_deliver_vault"
    )  # no 1Password vault drop (bootstrap self-fetches over mTLS)
    assert not hasattr(
        workflow, "step_rotate_admin_password"
    )  # rotation is a DEP-config concern
    # EACS wipes TCC, so the ScreenCapture grant has to be re-applied every cycle and must
    # come after the bootstrap (it needs the worker binaries and cltbld's console session).
    assert calls.index("sentinel") < calls.index("screencapture")  # bug 2073303
...
def test_wipe_aborts_when_idle_cannot_be_confirmed():
    ...
    with (
        ...
        patch("orchestrator.workflow.ssh.run") as run,
        patch(
            "orchestrator.workflow.taskcluster.is_currently_busy",
            side_effect=RuntimeError("Bad mac"),
        ),
        patch("orchestrator.workflow.simplemdm.wipe") as wipe,
    ):
        run.return_value.returncode = 0
