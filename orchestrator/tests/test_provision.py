...
def _cp(stdout: bytes = b"", returncode: int = 0) -> subprocess.CompletedProcess:
    return subprocess.CompletedProcess(
        args=["ssh"], returncode=returncode, stdout=stdout, stderr=b""
    )
...
class _Host:
    ...
    def run(self, _fqdn, command, **_kw):
        ...
        if "bootstraptoken" in command:
            body = (
                b"escrowed to server: YES"
                if self.escrowed
                else b"escrowed to server: NO"
            )
            return _cp(body)
        return _cp()
    ...
    def install(self):
        return (
            ...
            patch(
                "orchestrator.workflow.ssh.secure_token_status", return_value=self.token
            ),
...
def test_resolve_offline_makes_no_api_calls():
    ...
    with (
        patch("orchestrator.workflow.simplemdm.find_device_by_name") as mdm,
...
def test_preflight_allows_sip_on_when_asked():
    _preflight(
        _Host(sip="System Integrity Protection status: enabled."),
        require_sip_disabled=False,
    )
...
def test_preflight_skips_when_sshd_is_not_up():
    ...
    with patch(
        "orchestrator.workflow.ssh.wait_for_sshd", side_effect=TimeoutError("nope")
    ):
...
def test_provision_runs_the_steps_in_order_and_never_wipes():
    ...
    with (
        patch(
            "orchestrator.workflow.step_preflight",
            side_effect=lambda *a, **k: calls.append("preflight"),
        ),
        patch(
            "orchestrator.workflow.step_mint",
            side_effect=lambda *a, **k: calls.append("mint"),
        ),
        patch(
            "orchestrator.workflow.step_escrow_bst",
            side_effect=lambda *a, **k: calls.append("escrow"),
        ),
        patch("orchestrator.workflow.step_wait_for_bootstrap_pkg"),
        patch(
            "orchestrator.workflow.step_wait_for_sentinel",
            side_effect=lambda *a, **k: calls.append("sentinel"),
        ),
        patch(
            "orchestrator.workflow.step_screencapture_grant",
            side_effect=lambda *a, **k: calls.append("screencapture"),
        ),
        patch(
            "orchestrator.workflow.step_wipe",
            side_effect=AssertionError("provision must never wipe"),
        ),
        patch(
            "orchestrator.workflow.step_quarantine",
            side_effect=AssertionError("no pool calls"),
        ),
    ):
    ...
    # The grant runs last, after the bootstrap: it needs the worker binaries in place
    # and cltbld owning the console session, neither of which exists before puppet has
    # run. EACS wiped TCC, so skipping it returns a host that cannot screen-capture and
    # says nothing about it (bug 2073303).
    assert calls == ["preflight", "mint", "escrow", "sentinel", "screencapture"]
...
def test_provision_no_wait_stops_after_escrow():
    with (
        ...
        patch("orchestrator.workflow.step_screencapture_grant"),
...
def test_provision_stops_at_the_gate():
    ...
    with (
        patch(
            "orchestrator.workflow.step_preflight",
            side_effect=NotReadyError("macOS 26.1"),
        ),
...
def test_pkg_gate_passes_once_the_managed_install_lands():
    ...
    seen = [
        ...
        False,
        True,
    ]  # sentinel absent, then pkg absent twice, then present
    with (
        patch("orchestrator.workflow.ssh.file_exists", side_effect=seen),
...
def test_pkg_gate_skips_a_host_in_the_wrong_group():
    ...
    with (
        patch("orchestrator.workflow.ssh.file_exists", return_value=False),
        patch("orchestrator.workflow.get_settings") as settings,
    ):
...
def test_pkg_gate_is_a_noop_on_an_already_bootstrapped_host():
    ...
    # the check.
    with (
        patch("orchestrator.workflow.ssh.file_exists", return_value=True) as exists,
        patch("orchestrator.workflow.get_settings") as settings,
    ):
        _pkg_settings(settings)
        workflow.step_wait_for_bootstrap_pkg(_ctx())
...
def test_preflight_reports_the_pkg_but_does_not_gate_on_it():
    ...
    try:
        workflow.step_preflight(
            _ctx()
        )  # pkg absent (file_exists -> False), must not raise
    finally:
        for p in patches:
            p.stop()
...
def test_provision_gates_on_the_pkg_before_the_sentinel_wait():
    ...
    with (
        ...
        patch(
            "orchestrator.workflow.step_wait_for_sentinel",
            side_effect=lambda *a: calls.append("sentinel"),
        ),
    ):
        workflow.provision(HOST)
...
def test_quarantine_on_register_waits_then_quarantines_in_the_discovered_pool():
    ...
    with (
        ...
        patch(
            "orchestrator.workflow.taskcluster.find_registered_pool", side_effect=seen
        ),
        patch("orchestrator.workflow.get_settings") as settings,
...
def test_quarantine_on_register_raises_if_the_worker_never_appears():
    with (
        patch("orchestrator.workflow.tc_credentials", return_value=("cid", "tok")),
        patch(
            "orchestrator.workflow.taskcluster.find_registered_pool", return_value=None
        ),
        patch("orchestrator.workflow.get_settings") as settings,
        patch("orchestrator.workflow.step_quarantine") as quarantine,
    ):
...
def test_os_update_surfaces_a_refused_start_as_not_ready():
    ...
    def _run(_fqdn, command, **_kw):
        ...
        if "tail" in command:
            return _cp(
                b"[ERROR] admin holds no SecureToken, so it is not a volume owner"
            )
        return _cp(b"launched")
